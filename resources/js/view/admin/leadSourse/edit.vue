        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{ $t('sidebar.leadSourse') }}</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><router-link :to="{name: 'indexLeadSourse'}">وحده القياس</router-link></li>
                            <li class="breadcrumb-item active">{{ $t('sidebar.leadSourse') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
...
export default {
    ...
    setup(props){
        ...
        let getMeasure = () => {
            ...
            adminApi.get(`/v1/dashboard/leadSourse/${id.value}/edit`)
                .then((res) => {
                    let l = res.data.data;
                    addMeasure.data.name = l.leadSourse.name;
                })
                .catch((err) => {
    ...
    methods: {
        editMeasure(){
            ...
            if(!this.v$.$error){
                ...
                adminApi.put(`/v1/dashboard/leadSourse/${this.id}`,this.data)
                    .then((res) => {
