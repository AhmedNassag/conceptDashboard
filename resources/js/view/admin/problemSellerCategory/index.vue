                            ...
                            <div class="card-header pt-0">
                                <div class="row justify-content-between">
                                    ...
                                    <div class="col-5 row justify-content-end">
                                        <router-link v-if="permission.includes('problemSellerCategory create')" :to="{name: 'createProblemSellerCategory', params: {lang: locale || 'ar'}}" class="btn btn-custom btn-warning">
                                            {{ $t('global.Add') }}
                                        </router-link>
                                    </div>
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr v-for="(item,index) in targets"  :key="item.id">
                                        ...
                                        <td><span v-for="category in item.problemseller_categories" :key="category.id"> {{category.name}} <br> </span></td>
                                        <td>
                                            <router-link v-if="permission.includes('problemSellerCategory edit')" :to="{name: 'editProblemSellerCategory', params: {lang: locale || 'ar',id:item.id}}" class="btn btn-sm btn-success me-2">
                                                <i class="far fa-edit"></i>
                                            </router-link>
                                            <a href="#" @click="deleteTarget(item.id,item.user.name,index)" v-if="permission.includes('SellerCategory delete')" data-bs-target="#staticBackdrop" class="btn btn-sm btn-danger me-2">
...
export default {
    ...
    setup() {
        ...
        let getTarget = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/problemsellerCategory?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    console.log(l);
        ...
        function deleteTarget(id, incomeName, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/problemsellerCategory/${id}`)
                    .then((res) => {
