        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{ $t('global.ComplaintClients') }}</h3>
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ $t('global.ComplaintClients') }}</li>
                        </ul>
                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0">
                                    ...
                                    <tr v-for="(item,index) in jobs" :key="item.id">
                                        <td>{{ index + 1 }}</td>
                                        <td>{{ item.complaint.name }}</td>
                                        <td>{{ item.user.name }}</td>
                                        <td>{{ item.notes }}</td>
                                        <th>{{dateFormat(item.created_at)}}</th>
                                        <td>
                                            <router-link
                                                :to="{name: 'showComplaintClient',params:{id:item.id}}"
...
export default {
    ...
    setup() {
        ...
        let getJob = (page = 1) => {
            ...
            adminApi.get(`/v1/dashboard/complaintClient?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
                    jobsPaginate.value = l.complaints;
                    jobs.value = l.complaints.data;
                })
                .catch((err) => {
