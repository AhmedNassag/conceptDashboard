        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">{{ $t('global.problemCommissionPlanManagement') }}</h3>
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ $t('global.problemCommissionPlanManagement') }}</li>
                        </ul>
                    </div>
            ...
            <div class="row">
                <div class="col-md-6" v-for="item in targets" :key="item.id">
                    <div class="card">
                        <router-link v-if="permission.includes('problemtargetPlan read')" :to="{name: 'indexProblemTarget', params: {lang: locale || 'ar',id:item.id}}" class="btn btn-sm btn-category">
                            <h3>{{item.name}}</h3>
                        </router-link>
                    </div>
                </div>
    ...
    setup() {
        ...
        let getTarget = (page = 1) => {
            loading.value = true;
                ...
                adminApi.get(`/v1/dashboard/problemtargetPlan?page=${page}&search=${search.value}`)
                .then((res) => {
                    let l = res.data.data;
        ...
        function deleteTarget(id, incomeName, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    adminApi.delete(`/v1/dashboard/problemtargetPlan/${id}`)
                    .then((res) => {
